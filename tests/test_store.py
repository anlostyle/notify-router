...
def test_fresh_store_starts_without_notification_templates(tmp_path):
    store = Store(tmp_path)

    assert store.templates == []
    assert json.loads((tmp_path / "conf" / "notify_template.json").read_text(encoding="utf-8")) == {"template": []}
